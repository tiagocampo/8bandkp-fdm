...
- **ADR 0008** locks the four big design decisions: Lutchyn-Oreg sign-of-det, pairing_sign DRY public API, `conjg()` sufficient for class-D Peierls, replace 2D golden with minigap-pattern test.
...
3. **`conjg()` is sufficient for class-D Peierls** on hole block. No second `add_peierls_coo` call. Wire and dense-QW builders unify on this convention.
...
| Phase 1.10 symmetric Peierls removal breaks wire PHS oracle | Medium | High | Phase 1.10 includes `verify_wire_PHS_4_field_still_green` smoke test |
