...
1.0 T tolerance. The slim Pfaffian row is **live**: `bcrit_pfaffian = arg_B at mu ≈ 0.6601 (tol ±0.0001)` where the colormap's `z2` column first reads -1 (extracted from `output/z2_phase_diagram.dat`, ticket 05 of `.scratch/archive/bdg-evaluator-pfaffian/`); see
`docs/superpowers/specs/archive/2026-07-05-pr41-completion-design.md`
§3.4 / D5). **No value in this table is hand-edited**; if the
...
emitted by the lecture script and asserts the **4 active witnesses** (wire
1D curve, wire 2D colormap, dense QW, slim Pfaffian) agree within a 1.0 T
tolerance. The slim Pfaffian row is read live from the existing
`output/z2_phase_diagram.dat` colormap (ticket 05 of
`.scratch/archive/bdg-evaluator-pfaffian/`) — the floor honored by the
witness is `bdg_default_pfaffian_floor = 1.0e-12_dp` (SSOT, ticket 02).
Full Bloch-Pfaffian on a periodic supercell is U13 (separate scoped PR);
measure different aspects -- 1D curve vs 2D colormap vs dense-QW; see
the TOLERANCE_BCRIT_RANGE docstring for rationale).
...
3. **Slim projected Pfaffian witness** (Issue 07; live witness for U2
   close-out, ticket 04): at the critical point
   ...
   - **S2**: analytical projection onto bands 7-8 (conduction-band edge
     per the k·p block-table SSOT)
   - **Live gate witness**: the slim Pfaffian row in the §13.0
     reconciliation table is read live from the existing
     `output/z2_phase_diagram.dat` colormap (first B row where the
     `z2` column reads `-1` at $\mu \approx 0.6601$ (tol $\pm 0.0001$)).
     No new Fortran I/O is required.
   - **Floor**: `bdg_default_pfaffian_floor = 1.0e-12_dp` (SSOT in
     `bdg_observables.f90`, ticket 02 of
     `.scratch/archive/bdg-evaluator-pfaffian/`).

   **Figure**: Slim Pfaffian witness S1 vs S2 — *live witness for the
   4-witness acceptance gate; the wire_pfaffian row reads from
   `output/z2_phase_diagram.dat` z2 column at $\mu \approx 0.6601
   \pm 0.0001$, ticket 05. The full Bloch-Pfaffian B-sweep on a periodic
   supercell is U13 (separate scoped PR).*

### 13.7.5 Slim Pfaffian witness criterion

The slim Pfaffian is the **live** 4th witness of the L13 acceptance
gate. It is read from the existing $(B, \mu)$ colormap dataset
(`output/z2_phase_diagram.dat` produced by `compute_wire_bdg_gap_sweep`),
extracting `bcrit_pfaffian = B` at $\mu \approx 0.6601$ (tol $\pm 0.0001$)
where the colormap's `z2` column first reads $-1$. No new Fortran I/O is
required: the slim Pfaffian row reads from the existing colormap file.
The floor honored by the witness is
`bdg_default_pfaffian_floor = 1.0e-12_dp` (promoted to SSOT per ticket
02 of `.scratch/archive/bdg-evaluator-pfaffian/`).

**Seam shape** (`bdg_observables.f90`): the seam sibling
`eval_bdg_pfaffian_witness_csr(H_bdg_csr, Nbdg, params)` returns
`s2_sign ∈ {-1, 0, +1}` (a thin wrapper over
`wire_pfaffian_witness_sweep` per ticket 04). The QW+Kitaev rung
sibling `eval_bdg_kitaev_majorana(H_k_array, k_par_values)` returns
`majorana_number ∈ {-1, 0, +1}` (wrapping `kitaev_majorana_number`).

**Forward reference**: U13 swaps the slim witness for the full
Bloch-Pfaffian on a periodic supercell (separate scoped PR; CLAUDE.md
"Known Issues").
...
| BdG Majorana (wire slim Pfaffian) | InAs/GaAs wire | PASS | Slim Pfaffian witness (colormap-extracted, z2==-1 at mu≈0.6601) — live in 4-witness gate |
...
- `tests/integration/test_lecture_13_acceptance_gate.sh` — U12 4-witness gate (slim Pfaffian row live per ticket 05; colormap-extracted from `output/z2_phase_diagram.dat` z2 column at $\mu \approx 0.6601 \pm 0.0001$)
