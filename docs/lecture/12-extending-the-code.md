...
src/apps/
  ...
  main_gfactor.f90 (program gfactor)
    ...
    |-> eigensolver        (unified eigensolver: `make_eigensolver` factory → `dense_solver_t` / `feast_solver_t`)
...
src/math/
  ...
  linalg.f90                (zheevx wrapper, ILAENV, DLAMCH)
    |-> definitions
  eigensolver.f90           (unified eigensolver: `make_eigensolver` factory, polymorphic `eigensolver_base`; methods: DENSE/FEAST; modes: FULL/INDEX/ENERGY)
    |-> definitions
    |-> sparse_matrices
...
main.f90: k-vector sweep
  for each k-point:
    ...
    |
    v
  solver: `make_eigensolver(cfg%solver)` → `dense_solver_t` (bulk/QW) or `feast_solver_t` (wire)
    |
    v
