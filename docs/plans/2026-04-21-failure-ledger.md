...
| `FL-007` | suspect solver behavior | `docs/figures/qw_absorption_spectrum.png`; `qw_absorption_strained.png`; `qw_absorption_vs_width.png`; associated optics outputs | `06` | TE and TM curves are nearly identical even where the prose, selection rules, and nextnano-style benchmarks require clear polarization contrast. | This may indicate a polarization decomposition bug, a solver issue, or both. | Benchmark the optics path against nextnano/literature before keeping any strong TE/TM claims. | `open-high-risk` |
