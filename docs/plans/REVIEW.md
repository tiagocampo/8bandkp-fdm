...
Review date: 2026-08-08 (updated from 2026-06-27)
...
| 79 | 2026-06-14-001-feat-bdg-majorana-validation-plan | parent plan | IN PROGRESS | Status footer added 2026-07-06 (`status: in-progress` + per-unit breakdown), refreshed 2026-07-13 to reflect U2 close-out (Phase 26), refreshed 2026-08-08 to reflect U10 close-out (Phase 27). Units shipped via PR #40 + PR #41 + Phase 26 (`feat/bdg-u2-actual-ship` PR #42 merged 2026-07-26 to `main@257b3c4`) + Phase 27 (`feat/bdg-u10-pfaffian-phase-diagram` PR #43 ready-to-push, branch HEAD `da87b70` pushed 2026-08-08 to `origin`, 11 commits ahead of `main`): **U2 closed 2026-07-13** (per-point BdG evaluator seam at `bdg_observables.f90` with 3 faces: `eval_bdg_point` + `eval_bdg_pfaffian_witness_csr` + `eval_bdg_kitaev_majorana`; SSOT `bdg_default_pfaffian_floor`; rename `compute_z2_gap*` → `*_bhz_heuristic`; migrate `main_topology.f90:1371`; fix latent `wire_pfaffian_witness` omega(1:4,1:4) bug; 51/51 unit green per ctest-target count — the 52→51 drop is the ctest-target-vs-`@test`-subroutines count after retiring `test_wire_pfaffian_witness.pf` per ticket 02; see `feedback_ctest_counts_targets`), U3 (Pfaffian + Kitaev harness), U4 (hole-block unification, ADR 0007 `half_wire_integral=max(L,R)`), U5 (cross-builder PHS via `test_bdg_phs_at_finite_bx` + B.1 identity test), U6 (Sticlet polarization via A.3a wire-polarization emitter), U7 (dense-QW BdG rung via block-(2,1) structural+sign fix Task 1.8), U8 (sparse-wire zero-gap fix, PR #40), **U10 closed 2026-08-08**: T1a seam-magnitude out-arg `1ee9ba2` + T1b per-B min-|Pf| producer `0a62143` + T2 config-driven writer paths + sibling phase fixture `9de1cef` + T3 Pfaffian-native z2 convention `29e85c5` + T4 BLOCKED (non-flat z2 colormap physically unsatisfiable inside FEST envelope → U13 deferred, do NOT widen FEST window or manufacture closure regime) + T5 gate precondition + doc-drift cleanup `1a7767e` + `d17f75b` + T6 proxy at-floor detection `b6f6b05` + doc-drift cleanup `cd1d427` + PR #43 review close-outs P1 (`28ce1b0`, parser format mismatch) + P2 (`63611d3`, all-zero short-circuit). 51/51 unit + 3/3 wire_bdg regression + 2/2 slim_pfaffian regression + 1/1 degeneracy regression + 1/1 lecture 13 acceptance gate green. **U12 gate runs as 3-witness; slim Pfaffian row reserved for U13** (the 4-witness slim-Pfaffian row regression was reverted in `8e9128d` per ticket 05; gate runs 3-witness with `WITNESS_LABEL="3-witness"`, `TOL_BCRIT_RANGE=1.0`; live colormap-extracted 4-witness destination is deferred to U13 Bloch-Pfaffian). Partial: U11 (lecture 13 disclosures + slim-Pfaffian caption + new §13.7.5 landed; full revamp not done). **Still open: U1, U9.** Explicitly deferred: U13 (periodic/Bloch BdG construction — separate scoped PR per CLAUDE.md Known Issues). NOT archived. U2 follow-up scratch `.scratch/bdg-u2-actual-ship/` → `.scratch/archive/bdg-u2-finish/`; U10 scratch `.scratch/bdg-u10-pfaffian-phase-diagram/` → `.scratch/archive/bdg-u10-pfaffian-phase-diagram/` (both 2026-08-08). |
