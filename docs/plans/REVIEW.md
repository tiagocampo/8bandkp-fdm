...
| 79 | 2026-06-14-001-feat-bdg-majorana-validation-plan | parent plan | IN PROGRESS | Status footer added 2026-07-06 (`status: in-progress` + per-unit breakdown), refreshed 2026-07-13 to reflect U2 close-out (Phase 26). Units shipped via PR #40 + PR #41 + Phase 26 (`feat/bdg-u2-actual-ship` PR): **U2 closed 2026-07-13** (per-point BdG evaluator seam at `bdg_observables.f90` with 3 faces: `eval_bdg_point` + `eval_bdg_pfaffian_witness_csr` + `eval_bdg_kitaev_majorana`; SSOT `bdg_default_pfaffian_floor`; rename `compute_z2_gap*` → `*_bhz_heuristic`; migrate `main_topology.f90:1371`; fix latent `wire_pfaffian_witness` omega(1:4,1:4) bug; 51/51 unit green per ctest-target count — the 52→51 drop is the ctest-target-vs-`@test`-subroutines count after retiring `test_wire_pfaffian_witness.pf` per ticket 02; see `feedback_ctest_counts_targets`), U3 (Pfaffian + Kitaev harness), U4 (hole-block unification, ADR 0007 `half_wire_integral=max(L,R)`), U5 (cross-builder PHS via `test_bdg_phs_at_finite_bx` + B.1 identity test), U6 (Sticlet polarization via A.3a wire-polarization emitter), U7 (dense-QW BdG rung via block-(2,1) structural+sign fix Task 1.8), U8 (sparse-wire zero-gap fix, PR #40), **U12 gate runs as 3-witness; slim Pfaffian row reserved for U13** (the 4-witness slim-Pfaffian row regression was reverted in `8e9128d` per ticket 05; gate runs 3-witness with `WITNESS_LABEL="3-witness"`, `TOL_BCRIT_RANGE=1.0`; live colormap-extracted 4-witness destination is deferred to U13 Bloch-Pfaffian). Partial: U11 (lecture 13 disclosures + slim-Pfaffian caption + new §13.7.5 landed; full revamp not done). **Still open: U1, U9, U10.** Explicitly deferred: U13 (periodic/Bloch BdG construction — separate scoped PR per CLAUDE.md Known Issues). NOT archived. **Refreshed 2026-07-27 post-U10 execution on `feat/bdg-u10-pfaffian-phase-diagram` (HEAD `d17f75b`, 5 commits ahead main):** T1b per-B min-\|Pf\| proxy + T2 config-driven writer paths + sibling phase fixture + T3 Pfaffian-native z2 column `{-1,+1,0}` + T5 gate precondition update (full cleanup, 2 commits `1a7767e` + `d17f75b`) + **T6 slim Pfaffian per-B `\|Pf\|_min` saturation detection** (at-floor branch → approximation label; module-level `_PFAFFIAN_DEGENERACY_TOL=1e-6` relative-variance threshold at `lecture_13_topological.py:43-52`); 51/51 unit + wire-BdG + slim_Pfaffian regression green. **T4 (2026-07-27) closed BLOCKED:** non-flat z2 colormap assertion physically unsatisfiable inside slim Pfaffian's signal regime (probes 1–5 enumerated in `tickets/T4_regression_test.md` §Probe findings: 231/231 cells `s2_sign=-1` inside FEST envelope); deferred to U13 (full Bloch-Pfaffian + periodic Peierls-twist). Do NOT widen FEST window (ADR 0005 out-of-scope) or manufacture closure regime (would bug working physics). T5 closed 2026-07-27 (full cleanup). T6 closed 2026-07-27 (per-B `\|Pf\|_min` saturation detection, grilling resolution (c)→(a)). Full Bloch-Pfaffian remains U13 destination. PR #43 review (T1b + T2 + T3) parallel track; T4/T5/T6 not in PR scope. |
