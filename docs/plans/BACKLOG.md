...
Phases 1-25 + Phase 26 complete (Phase 26 = U2 close-out: seam siblings + slim Pfaffian
plug-in + heuristic retirement, follow-up commits on `feat/bdg-u2-actual-ship`,
ctest 51/51 unit green). PR #41 merged to `main@8fa9551` 2026-07-13; U2
follow-up PR #42 still pending merge on `feat/bdg-u2-actual-ship`
(branch HEAD `4986d5a`).
...
still in progress — **U1, U9, U10 open; U2 close-out on `feat/bdg-u2-actual-ship` (PR #42 pending merge)**; U13 deferred.
...
| 26. U2 close-out — BdG evaluator Pfaffian plug-in | — | Branch `feat/bdg-u2-actual-ship` follow-up to PR #42 (pending merge), HEAD `4986d5a`. Seam siblings `eval_bdg_pfaffian_witness_csr` (wire rung, slim projected Pfaffian via `wire_pfaffian_witness_sweep`, ticket 04; retyped to `bdg_pfaffian_params_t` ticket 01) + `eval_bdg_kitaev_majorana` (QW+Kitaev rung, wraps `kitaev_majorana_number`) on `bdg_observables.f90`. SSOT `bdg_pfaffian_params_t` + factory `bdg_pfaffian_params_with_floor` (default `bdg_default_pfaffian_floor`, ticket 01); orphan dense `wire_pfaffian_witness` + helpers retired (ticket 02). Rename `compute_z2_gap*` → `*_bhz_heuristic` (ticket 03). Migrate `main_topology.f90:1371` to seam. Fix latent `wire_pfaffian_witness` omega(1:4,1:4) all-zeros bug (retained in CSR sweep). Test contracts tightened to `s2 ∈ {-1, 0, +1}` + User Story 5 meta-test (ticket 03); dense `test_wire_pfaffian_witness.pf` retired (ticket 02) ⇒ 51/51 unit green (ctest-target count). 3-witness acceptance gate (4-witness slim-Pfaffian row reverted in 8e9128d, reserved for U13, ticket 05). Polarization verifier colormap SKIP precondition. AGENTS.md inventory + DAG updated (`bdg_pfaffian_params_t`). Lesson doc + memory entry. **PR #42 MERGED 2026-07-26 (squash `257b3c4`) → U2 closed.** **U10 execution on `feat/bdg-u10-pfaffian-phase-diagram` (HEAD `d17f75b`, ahead 5 commits): T1b per-B min-\|Pf\| proxy + T2 config-driven writer paths + sibling phase fixture + T3 Pfaffian-native z2 column `{-1,+1,0}` + T5 gate precondition update (full cleanup, 2 commits `1a7767e` + `d17f75b`); 51/51 unit + 5/5 wire-BdG regression green. **T4 (2026-07-27) closed BLOCKED:** non-flat z2 colormap assertion physically unsatisfiable inside slim Pfaffian's signal regime (probes 1–5 enumerated in `tickets/T4_regression_test.md` §Probe findings: 231/231 cells `s2_sign=-1` inside FEST envelope); deferred to U13 (full Bloch-Pfaffian + periodic Peierls-twist). Do NOT widen FEST window (ADR 0005 out-of-scope) or manufacture closure regime (would bug working physics). T5 active (gate precondition update `z2==-1` native per T3 standing decision) → **closed 2026-07-27 (full cleanup, 2 commits).** **T6 (2026-07-27) closed:** slim Pfaffian per-B `\|Pf\|_min` saturation detection at `lecture_13_topological.py` — file-present-but-all-at-floor branch falls back to approximation label (non-numeric, gate auto-detects → 3-witness fallback at `test_lecture_13_acceptance_gate.sh:86`); module-level `_PFAFFIAN_DEGENERACY_TOL=1e-6` relative-variance threshold. Empirical baseline (`wire_inas_gaas_bdg_topological_phase.toml` sibling, nB=6 B∈[0,5]T): all 6 per-B `\|Pf\|_min = 4.0E-08` (FEST precision floor) → relative variance = 0.0e+00, branch fires correctly. Three test paths confirmed: file absent → existing approximation WARN (T5); file saturated → new degenerate WARN; file varying → numeric `BCRIT wire_pfaffian <B>`. Full Bloch-Pfaffian remains U13 — T6 unlocks the lecture 13 evidence path without manufacturing a fake `bcrit_pfaffian` number that would duplicate `bcrit_2d`. PR #43 review (T1b + T2 + T3) parallel track; T4/T5/T6 not in PR scope. | U2 MERGED, U10 in progress, T4 BLOCKED→U13, T6 closed |

**Phases 1-25 + Phase 26 (U2 close-out, PR #42 follow-up) complete-pending-merge.** 145+ tests pass (BdG regression suite + acceptance-gate label + 51/51 unit including 6 seam-sibling tests in `test_bdg_pfaffian_witness_csr.pf`). Phase 21 in progress (PR #35). Phases 18-19 + 22 are active backlog (architecture deepening + FEAST parity). Parent BdG validation plan still in progress — U1, U9, U10 open; **U2 close-out on `feat/bdg-u2-actual-ship` (PR #42 open)** (Phase 26); U13 deferred per CLAUDE.md Known Issues; U6 verifier @todo U13 BLOCKING-EMPIRICAL documented per option (b).
