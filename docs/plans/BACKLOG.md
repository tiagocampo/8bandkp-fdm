...
follow-up PR #42 still landing review findings on `feat/bdg-u2-actual-ship`.
...
still in progress — **U1, U9, U10 open; U2 close-out on `feat/bdg-u2-actual-ship` (PR #42 open)**; U13 deferred.
...
| 26. U2 close-out — BdG evaluator Pfaffian plug-in | — | Branch `feat/bdg-u2-actual-ship` follow-up to PR #42 (open), HEAD `8e9128d`. Seam siblings `eval_bdg_pfaffian_witness_csr` (wire rung, slim projected Pfaffian via `wire_pfaffian_witness_sweep`, ticket 04; retyped to `bdg_pfaffian_params_t` ticket 01) + `eval_bdg_kitaev_majorana` (QW+Kitaev rung, wraps `kitaev_majorana_number`) on `bdg_observables.f90`. SSOT `bdg_pfaffian_params_t` + factory `bdg_pfaffian_params_with_floor` (default `bdg_default_pfaffian_floor`, ticket 01); orphan dense `wire_pfaffian_witness` + helpers retired (ticket 02). Rename `compute_z2_gap*` → `*_bhz_heuristic` (ticket 03). Migrate `main_topology.f90:1371` to seam. Fix latent `wire_pfaffian_witness` omega(1:4,1:4) all-zeros bug (retained in CSR sweep). Test contracts tightened to `s2 ∈ {-1, 0, +1}` + User Story 5 meta-test (ticket 03); dense `test_wire_pfaffian_witness.pf` retired (ticket 02) ⇒ 51/51 unit green (ctest-target count). 3-witness acceptance gate (4-witness slim-Pfaffian row reverted in 8e9128d, reserved for U13, ticket 05). Polarization verifier colormap SKIP precondition. AGENTS.md inventory + DAG updated (`bdg_pfaffian_params_t`). Lesson doc + memory entry. | IN PROGRESS (PR #42 open) |
