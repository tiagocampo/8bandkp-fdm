...
Consolidated from REVIEW.md on 2026-05-06. Updated 2026-08-08.
Piezoelectric explicitly excluded (ZB [001] = zero by symmetry, wires = negligible).
Phases 1-25 + Phase 26 + Phase 27 complete (Phase 26 = U2 close-out: seam siblings
+ slim Pfaffian plug-in + heuristic retirement, PR #42 merged 2026-07-26 to
`main@257b3c4`; Phase 27 = U10 close-out: per-B min-|Pf| proxy + Pfaffian-native
z2 + non-flat z2 colormap deferred to U13, PR #43 ready-to-push on
`feat/bdg-u10-pfaffian-phase-diagram` HEAD `da87b70`). ctest 51/51 unit +
3/3 wire_bdg regression + 2/2 slim_pfaffian regression + 1/1 degeneracy
...
still in progress — **U1, U9 open; U2 merged, U10 closed, U13 deferred**.
...
## Phase 21: COMPLETED (PR #35 merged 2026-06-05)

Publishable validation benchmarks — ISBT oscillator strength, SC Schrödinger-Poisson, exciton Baston reference. Branch `feat/publishable-benchmarks-phase21` merged at `e52a2702`; 6 follow-up issues (#29–#34) separated as own scope per BACKLOG "Minor Carry-Over Items".
...
- ~~U1, U2, U9, U10, U11, U12 of parent BdG validation plan still open~~ → **UPDATED 2026-07-13** (Phase 26): U2 shipped via `bdg_observables.f90` seam (3 faces: `eval_bdg_point` + `eval_bdg_pfaffian_witness_csr` + `eval_bdg_kitaev_majorana`); U12 gate runs as **3-witness** (the 4-witness slim-Pfaffian row live destination per ticket 05 was reverted in `8e9128d`; slim-Pfaffian row reserved for U13 Bloch-Pfaffian; colormap-extracted `bcrit_pfaffian` criterion remains spec-of-record in `.scratch/archive/bdg-evaluator-pfaffian/spec.md`); U11 partial (lecture disclosures + slim-Pfaffian caption + new §13.7.5); **U1, U9, U10 still open**; U13 still explicitly deferred per CLAUDE.md Known Issues (separate scoped PR).
...
- Seam siblings on `bdg_observables.f90`: `eval_bdg_pfaffian_witness_csr` (CSR BdG → s2_sign, wire-rung invariant via slim projected Pfaffian; thin wrapper over `wire_pfaffian_witness_sweep`, ticket 04; retyped to take `bdg_pfaffian_params_t` per ticket 01) and `eval_bdg_kitaev_majorana` (H_k_array + k_par_values → majorana_number, QW+Kitaev rung; wraps `kitaev_majorana_number`).
- SSOT `bdg_pfaffian_params_t` + factory `bdg_pfaffian_params_with_floor` (default `bdg_default_pfaffian_floor = 1.0e-12_dp`) wired from default to call site per ticket 01; orphan dense `wire_pfaffian_witness` + `s1_project`/`s2_construct` helpers retired per ticket 02 (the 2 dense-subroutine literal sites at `:1680`/`:1698` deleted with the subroutine; only the CSR sweep `:1781` site consumes the floor).
- Rename `compute_z2_gap`/`compute_z2_gap_edge` → `*_bhz_heuristic` (ticket 03 — BHZ-only scope-narrow signal at the call site).
- Migrate `main_topology.f90:1371` from `wire_pfaffian_witness_sweep` (helper) to seam sibling `eval_bdg_pfaffian_witness_csr`, passing `bdg_pfaffian_params_t(pfaffian_floor = bdg_default_pfaffian_floor)`.
- Fix latent bug in `wire_pfaffian_witness` (omega(1:4,1:4) extraction yielded all-zeros; replaced with proper local 4×4 omega matching the CSR sweep variant) — then the dense variant was retired (ticket 02); the fix lives on in the CSR sweep.
- Strict sign-agreement contract in `test_bdg_pfaffian_witness_csr.pf` GREEN via non-diagonal fixture (H(7,8)=H(8,7)=0.3 inside the 4×4 projection ⇒ Pf_real = -0.0225 ⇒ s2_sign = -1) per ticket 03; strict `s1 == s2` form loosened to gate-relevant `s2 ∈ {-1, 0, +1}` with `s2 /= 0` because S1 strategy cannot produce non-zero Pf for real-symmetric single-particle BdG with imaginary diagonal pairing (Issue 05, deferred); meta-test `test_pfaffian_witness_spec_user_story_5_contract` pins the User Story 5 contract.
- New tests: `test_bdg_pfaffian_witness_csr.pf` (6 tests: delegation + zero-matrix + range + nondiagonal + floor-consumption + User Story 5 meta-test, per ticket 03), `test_bdg_kitaev_majorana.pf` (3 tests, range + topological -1 + trivial +1), `test_bdg_evaluator.pf` SSOT-pin tests re-routed through `bdg_pfaffian_params_t`, 1 seam-sibling call added to `test_kitaev_majorana.pf`. (Dense `test_wire_pfaffian_witness.pf` retired with the subroutine per ticket 02.)
- 3-witness acceptance gate (slim-Pfaffian row reserved for U13 Bloch-Pfaffian, ticket 05): the 4-witness slim-Pfaffian row regression was reverted in `8e9128d`; the gate runs as 3-witness (`wire_curve`, `wire_2d`, `wire_pfaffian` witnesses), `WITNESS_LABEL="3-witness"`, `TOL_BCRIT_RANGE=1.0`. Slim-Pfaffian site-by-site sweep stays as an interim diagnostic awaiting the U13 periodic/Bloch construction.
- Polarization verifier SKIP precondition parses the colormap; exits non-zero if the `z2==-1` row is missing (gate witness re-derived from existing Fortran output per ticket 05).
- AGENTS.md inventory + DAG row reflect seam siblings + `bdg_pfaffian_params_t` SSOT + L0 deps + Layer-3 delegation edge (one symbol only, per ticket 04).
...
ctest 51/51 unit tests pass (the 52→51 drop is the ctest-target-vs-`@test`-subroutines count after retiring `test_wire_pfaffian_witness.pf`; see `feedback_ctest_counts_targets`). Build clean. Follow-up PR #42 still landing review findings on `feat/bdg-u2-actual-ship` HEAD `8e9128d` (PR #42 open, not yet merged).

**User Story 5 (verbatim, spec-of-record at `.scratch/archive/bdg-evaluator-pfaffian/spec.md:95`; test: `test_pfaffian_witness_spec_user_story_5_contract` in `tests/unit/test_bdg_pfaffian_witness_csr.pf`):** "As a researcher running cross-builder BdG unit tests, I want the Pfaffian sibling's witness assertions to be GREEN today (independent of the deferred Bloch-periodic BdG construction U13), where the witness is the S2-projected Pfaffian sign `s2 ∈ {-1, 0, +1}` (range contract pinned by `s2 >= -1 .and. s2 <= 1` and non-zeroness pinned by `s2 /= 0` on a non-diagonal synthetic fixture). Full S1+S2 strict sign-agreement is the Majorana-basis Pfaffian problem (Issue 05), deferred to a follow-up scope — for U2 the S1 path returns `(0, -1)` on the canonical fixtures, which is a structurally-valid gap-closure signal in the S1 eigenspace, not a defect."
...
**Branch push + archive (2026-08-08):** `feat/bdg-u10-pfaffian-phase-diagram` pushed to `origin` (HEAD `da87b70`, 11 commits ahead of `main@257b3c4`); PR #43 body refreshed to reflect full U10 close-out + P1+P2 review findings; scratch dir `.scratch/bdg-u10-pfaffian-phase-diagram/` archived to `.scratch/archive/bdg-u10-pfaffian-phase-diagram/`. Parent plan header `updated: 2026-08-08`. REVIEW row 79 updated. U2 follow-up scratch `.scratch/bdg-u2-actual-ship/` (PR #42 merged 2026-07-26 to `main@257b3c4`) archived to `.scratch/archive/bdg-u2-finish/` per its `design.md` footer.
...
| 21. Publishable benchmarks | — | ISBT 6-config sweep (42 checks), SC benchmark (5 checks), exciton tightening + Bastard 1982 | **DONE (PR #35 merged 2026-06-05 at `e52a2702`; 6 follow-up issues #29–#34 separate-scope)** |
...
| 26. U2 close-out — BdG evaluator Pfaffian plug-in | — | Branch `feat/bdg-u2-actual-ship` follow-up to PR #42 (MERGED 2026-07-26, squash `257b3c4`). Seam siblings `eval_bdg_pfaffian_witness_csr` (wire rung, slim projected Pfaffian via `wire_pfaffian_witness_sweep`, ticket 04; retyped to `bdg_pfaffian_params_t` ticket 01) + `eval_bdg_kitaev_majorana` (QW+Kitaev rung, wraps `kitaev_majorana_number`) on `bdg_observables.f90`. SSOT `bdg_pfaffian_params_t` + factory `bdg_pfaffian_params_with_floor` (default `bdg_default_pfaffian_floor`, ticket 01); orphan dense `wire_pfaffian_witness` + helpers retired (ticket 02). Rename `compute_z2_gap*` → `*_bhz_heuristic` (ticket 03). Migrate `main_topology.f90:1371` to seam. Fix latent `wire_pfaffian_witness` omega(1:4,1:4) all-zeros bug (retained in CSR sweep). Test contracts tightened to `s2 ∈ {-1, 0, +1}` + User Story 5 meta-test (ticket 03); dense `test_wire_pfaffian_witness.pf` retired (ticket 02) ⇒ 51/51 unit green (ctest-target count). 3-witness acceptance gate (4-witness slim-Pfaffian row reverted in 8e9128d, reserved for U13, ticket 05). Polarization verifier colormap SKIP precondition. AGENTS.md inventory + DAG updated (`bdg_pfaffian_params_t`). Lesson doc + memory entry. **PR #42 MERGED 2026-07-26 (squash `257b3c4`) → U2 closed.** Scratch archived 2026-08-08: `.scratch/bdg-u2-actual-ship/` → `.scratch/archive/bdg-u2-finish/`. | DONE |
| 27. U10 close-out — Pfaffian phase diagram from wire path | — | Branch `feat/bdg-u10-pfaffian-phase-diagram` PR #43 ready-to-push (HEAD `da87b70`, 11 commits ahead of `main@257b3c4`). T1a seam-magnitude out-arg `1ee9ba2` + T1b per-B min-\|Pf\| producer `0a62143` + T2 config-driven writer paths + sibling phase fixture `9de1cef` + T3 Pfaffian-native z2 column `{-1,+1,0}` `29e85c5` + T4 BLOCKED (non-flat z2 colormap physically unsatisfiable inside FEST envelope → U13 deferred) + T5 gate precondition + doc-drift cleanup `1a7767e` + `d17f75b` + T6 proxy at-floor detection `b6f6b05` + doc-drift cleanup `cd1d427` + PR #43 review close-outs P1 (`28ce1b0`, parser format mismatch) + P2 (`63611d3`, all-zero short-circuit). 51/51 unit + 3/3 wire_bdg regression + 2/2 slim_pfaffian regression + 1/1 degeneracy regression (test_pfaffian_degeneracy_detection.py 5/5 PASS) + 1/1 lecture 13 acceptance gate green. Full Bloch-Pfaffian remains U13 (BLOCKING-EMPIRICAL per CLAUDE.md Known Issues). Pushed to `origin` 2026-08-08; PR #43 body refreshed. Scratch archived 2026-08-08: `.scratch/bdg-u10-pfaffian-phase-diagram/` → `.scratch/archive/bdg-u10-pfaffian-phase-diagram/`. | DONE |

**Phases 1-25 + Phase 26 (U2 close-out) + Phase 27 (U10 close-out) + Phase 21 (publishable benchmarks, PR #35 merged 2026-06-05) complete.** 145+ tests pass (BdG regression suite + acceptance-gate label + 51/51 unit including 6 seam-sibling tests in `test_bdg_pfaffian_witness_csr.pf` + 1/1 degeneracy regression + 6 ISBT/SC-SP/exciton Phase 21 benchmarks). Phases 18-19 + 22 are active backlog (architecture deepening + FEAST parity). Parent BdG validation plan still in progress — **U1, U9 open; U2 MERGED, U10 closed, U13 deferred per CLAUDE.md Known Issues**; U6 verifier @todo U13 BLOCKING-EMPIRICAL documented per option (b).
