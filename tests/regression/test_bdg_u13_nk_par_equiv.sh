#!/bin/bash
# U13 T3: nk_par=1 vs absent regression guard. Runs two committed wire-bdg
# 2D fixtures: one with [bdg].nk_par ABSENT (U10 default path), one with
# [bdg].nk_par = 1 (T3 explicit knob). Diffs every emitted output file
# byte-for-byte. Guard FAILS if dispatch diverges.
#
# Args: <topologicalAnalysis_exe> <config_absent> <config_nk1>
set -euo pipefail
EXE="$(realpath "$1")"
CONFIG_ABSENT="$(realpath "$2")"
CONFIG_NK1="$(realpath "$3")"
...
RUN1="$WORKDIR/run1"
mkdir -p "$RUN1"
/bin/cp "$CONFIG_ABSENT" "$RUN1/input.toml"
cd "$RUN1"
...
RUN2="$WORKDIR/run2"
mkdir -p "$RUN2"
/bin/cp "$CONFIG_NK1" "$RUN2/input.toml"
cd "$RUN2"
OMP_NUM_THREADS=4 "$EXE" > run.log 2>&1 || {
