...
Generates an illustrative Sticlet electron-hole coherence P_M(n) plot
showing the textbook topological vs trivial profile, because the
existing dense-QW BdG rung verifier does not emit BdG eigenvectors
(the wire `extract_edge_states` path emits only localization lengths).

The synthetic profiles reproduce the canonical signatures:
- Topological: P_M saturates at 1 at the two ends, ~0 in the bulk,
  half-wire integral >= 0.4
- Trivial: P_M is small everywhere (~0), half-wire integral ~0

The same canonical formulas are exercised by the pFUnit test
test_majorana_polarization.pf on real eigenvectors; this script is
the visualization companion, not a new physics witness.
"""
import os
import sys
