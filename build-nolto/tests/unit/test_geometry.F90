...
#line 319 "/data/8bandkp-fdm/tests/unit/test_geometry.pf"
  ...
  subroutine test_wire_rect_single_material()
    ...
    cfg%wire%regions(1)%outer = 100.0_dp  ! larger than any distance
...
#line 379 "/data/8bandkp-fdm/tests/unit/test_geometry.pf"
  ...
  subroutine test_wire_rect_two_regions()
    ...
    allocate(cfg%wire%regions(2))
    cfg%wire%regions(1)%material = 'InAs'
    cfg%wire%regions(1)%inner = 0.0_dp
    cfg%wire%regions(1)%outer = 4.0_dp    ! core: r <= 4
    cfg%wire%regions(2)%material = 'GaSb'
    cfg%wire%regions(2)%inner = 4.0_dp
    cfg%wire%regions(2)%outer = 100.0_dp  ! shell: r > 4
