...
contains
  ...
  ! ==============================================================================
  ! Compute LDOS for a Bogoliubov-de Gennes (BdG) Hamiltonian.
  !
  ! The BdG Hamiltonian is a 2N x 2N matrix in Nambu space:
  !   H_BdG = [ H_0   Delta ]
  !           [ Delta*  -H_0* ]
  !
  ! The LDOS formula is the same: LDOS = -(1/pi) * Im[G(r,r,E+ieta)]
  ! but G now includes both particle and hole contributions.
  !
  ! Args:
    !   H_bdg     -- CSR BdG Hamiltonian (2N x 2N, complex)
    !   E          -- energy (real, eV)
    !   eta        -- broadening (real, eV)
    !   ldos       -- output LDOS at each Nambu site (real, 1/eV)
  ! ==============================================================================
  subroutine compute_ldos_bdg(H_bdg, E, eta, ldos)
    type(csr_matrix), intent(in) :: H_bdg
    real(kind=dp), intent(in) :: E
    real(kind=dp), intent(in) :: eta
    real(kind=dp), intent(out) :: ldos(:)
    ...
    if (N /= size(ldos)) then
      print *, 'ERROR: compute_ldos_bdg: ldos size mismatch (N=', N, ', ldos=', size(ldos), ')'
      stop 1
    end if

    shift = cmplx(E, eta, kind=dp)

    ! Copy CSR and apply shift: A = E + i*eta - H_bdg
    nnz = H_bdg%nnz
    allocate(a_val(nnz), ia(N+1), ja(nnz))
    a_val = shift - H_bdg%values
    ia = H_bdg%rowptr
    ja = H_bdg%colind
    ...
    call pardiso_c(pt, maxfct, mnum, mtype, phase, N, &
                   ...
                   rhs, sol, error_loc)
    if (error_loc /= 0) then
      print *, 'ERROR: compute_ldos_bdg: symbolic factorization failed (error=', error_loc, ')'
      phase = -1
      call pardiso_c(pt, maxfct, mnum, mtype, phase, N, &
                     a_val, ia, ja, perm, nrhs, iparm, msglvl, &
                     rhs, sol, error_loc)
    ...
    call pardiso_c(pt, maxfct, mnum, mtype, phase, N, &
                   ...
                   rhs, sol, error_loc)
    if (error_loc /= 0) then
      print *, 'ERROR: compute_ldos_bdg: numeric factorization failed (error=', error_loc, ')'
      phase = -1
      call pardiso_c(pt, maxfct, mnum, mtype, phase, N, &
                     a_val, ia, ja, perm, nrhs, iparm, msglvl, &
                     rhs, sol, error_loc)
    ...
    do r = 1, N
      ...
      call pardiso_c(pt, maxfct, mnum, mtype, phase, N, &
                     ...
                     rhs, sol, error_loc)
      if (error_loc /= 0) then
        print *, 'ERROR: compute_ldos_bdg: solve failed at r=', r, ' (error=', error_loc, ')'
        phase = -1
        call pardiso_c(pt, maxfct, mnum, mtype, phase, N, &
                       a_val, ia, ja, perm, nrhs, iparm, msglvl, &
                       rhs, sol, error_loc)
    ...
    deallocate(a_val, ia, ja, perm, rhs, sol)
  end subroutine compute_ldos_bdg
