module strain_pde
  ...
  ! ==============================================================================
  ! Navier-Cauchy elasticity PDE solver for wire plane-strain (Issue #06,
  ! ADR 0005).
  !
  ! Split out of strain_solver.f90 along the concern boundary identified
  ! in ADR 0005: this module owns ONLY the 2D displacement PDE (stiffness
  ! assembly + MKL PARDISO solve + strain-from-displacement recovery).
  ! The Bir-Pikus formulas, strain table, and QW biaxial strain remain in
  ! strain_solver.f90; the strain_result type lives in the leaf module
  ! strain_types.f90 (shared by both, avoiding a circular `use`).
  !
  ! Cross-concern SAVE coupling resolved: the original strain_solver.f90
  ! shared a single module-level `warned_invalid_mat` flag across the
  ! displacement-PDE concern and the Bir-Pikus concern. After the split
  ! each concern encapsulates its own warn-once flag. The worst-case
  ! behavioural change is that an invalid material_id in wire mode now
  ! emits at most two warning lines (one per concern) instead of one;
  ! the physics (skip invalid grid points) is unchanged.
  !
  ! DOF ordering (wire cross-section is the x-y grid):
  !   comp=1 is u_y (grid x-direction -> PDE y axis)
  !   comp=2 is u_z (grid y-direction -> PDE z axis)
  ! ==============================================================================

  use definitions, only: dp, paramStruct, spatial_grid, grid_ngrid
  use linalg, only: pardiso_real
  use strain_types, only: strain_result
  use, intrinsic :: iso_c_binding, only: c_intptr_t

  implicit none

  ! Module-level flag to warn about invalid material_id only once
  ! (encapsulated in this concern; see module header).
  logical, save :: warned_invalid_mat_pde = .false.

  private

  public :: compute_strain_wire

contains

  ! ==================================================================
  ! Wire plane-strain PDE: solve Navier-Cauchy elasticity on the 2D
  ! cross-section for the in-plane displacement (u_y, u_z) under the
  ! axial lattice-mismatch source, then recover the strain tensor.
  !
  ! Stiffness matrix assembled in COO (real, face-fraction-weighted
  ! averages at material interfaces), converted to CSR, solved with
  ! MKL PARDISO (real unsymmetric, mtype=11 — cross-derivative face
  ! weights make the matrix nonsymmetric at interfaces).
  !
  ! Body is byte-for-byte the historical compute_strain_wire in
  ! strain_solver.f90; only its module home changed.
  ! ==================================================================
  subroutine compute_strain_wire(grid, params, material_id, a0_ref, strain_out)
    ...
    if (error /= 0) then
      ...
      error stop 'compute_strain_wire: PARDISO solve failed'
    end if
  ...
  ! ==================================================================
  ! Private helpers (moved verbatim from strain_solver.f90)
  ! ==================================================================
  ...
  subroutine add_coo_entry(rows, cols, vals, capacity, idx, row, col, val)
    ...
    if (idx > capacity) then
      ...
      error stop 'add_coo_entry: COO capacity exceeded'
    end if
    rows(idx) = row
    cols(idx) = col
    vals(idx) = val
