module gfactorFunctions
  ...
  public :: gfactorCalculation
  public :: gfactorCalculationNumerical
  public :: NumericalConfig
  public :: METHOD_ANALYTICAL, METHOD_NUMERICAL
...
subroutine gfactorCalculationNumerical(tensor, whichBand, bandIdx, numcb, numvb, &
  ...
  complex(kind=dp), allocatable, dimension(:,:) :: H_plus, H_minus
  complex(kind=dp), allocatable, dimension(:) :: eigenvectors_plus, eigenvectors_minus
  real(kind=dp), allocatable, dimension(:) :: eigenvalues_plus, eigenvalues_minus
  integer :: i, d, info
  ...
  if (whichBand == 0) then ! conduction band
    target_state_idx = bandIdx
  else ! valence band
    target_state_idx = numcb + bandIdx
  end if
  ...
  do d = 1, 3
    ...
    ! Allocate Hamiltonians for ±δB perturbations
    allocate(H_plus(dimax, dimax))
    allocate(H_minus(dimax, dimax))
    allocate(eigenvalues_plus(dimax))
    allocate(eigenvalues_minus(dimax))
    allocate(eigenvectors_plus(dimax))
    allocate(eigenvectors_minus(dimax))

    ! Build perturbed Hamiltonians with ±δB in specified direction
    call build_perturbed_hamiltonian(H_plus, dimax, field_dir, +delta_B, &
      & k_vec, nlayers, params, profile, kpterms, startz, endz, dz)

    call build_perturbed_hamiltonian(H_minus, dimax, field_dir, -delta_B, &
      & k_vec, nlayers, params, profile, kpterms, startz, endz, dz)
    ...
    if (info /= 0) then
      error_info = create_error(ERROR_EIGENVALUE_FAILURE, &
        "Eigenvalue solver failed for +δB Hamiltonian", &
        "gfactorCalculationNumerical", SEVERITY_ERROR, .false., &
        "Target state: " // trim(itoa(target_state_idx)) // ", Direction: " // field_dir)
      call handle_error(error_info)
      tensor(:,:,d) = (0.0_dp, 0.0_dp)
      goto 100  ! Skip to cleanup
    end if

    call solve_hamiltonian_numerical(H_minus, dimax, eigenvalues_minus, eigenvectors_minus, &
      & num_config%use_sparse_solver, info)
    ...
    ! Extract target state energies
    E_plus = eigenvalues_plus(target_state_idx)
    E_minus = eigenvalues_minus(target_state_idx)

    ! Calculate numerical g-factor using central finite difference
    ! g = (1/μ_B) * (dE/dB) ≈ (E_plus - E_minus) / (2 * μ_B * δB)
    g_factor_num = (E_plus - E_minus) / (2.0_dp * mu_B * delta_B)

    ! Store result in tensor (assuming 2x2 spin space for simplicity)
    ! In full implementation, this would handle the complete spin structure
    tensor(1,1,d) = cmplx(g_factor_num, 0.0_dp)
    tensor(2,2,d) = cmplx(g_factor_num, 0.0_dp)

    if (num_config%verbose_output) then
      print *, 'E_plus:', E_plus, 'E_minus:', E_minus
      print *, 'Numerical g-factor (', field_dir, '):', g_factor_num
    end if

100 continue ! Cleanup label

    ! Deallocate arrays
    deallocate(H_plus)
    deallocate(H_minus)
    deallocate(eigenvalues_plus)
    deallocate(eigenvalues_minus)
    deallocate(eigenvectors_plus)
    deallocate(eigenvectors_minus)
...
subroutine build_perturbed_hamiltonian(H, dimax, field_dir, delta_B, k_vec, &
  ...
  ! For now, build the standard Hamiltonian
  ! TODO: Add magnetic field perturbation based on field_dir and delta_B
  if (nlayers == 1) then
    call ZB8bandBulk(H, k_vec, params(1), g='g')
  else
    call ZB8bandQW(H, k_vec, profile, kpterms, g='g')
  end if

  ! TODO: Add Zeeman term with magnetic field perturbation
  ! This requires extending the Hamiltonian construction to include
  ! magnetic field dependence in the appropriate matrix elements
...
subroutine solve_hamiltonian_numerical(H, dimax, eigenvalues, eigenvectors, use_sparse, info)
  ...
  ! TODO: Implement sparse ARPACK solver option
  ...
  allocate(work(1))
  call zheev('V', 'U', dimax, H_work, dimax, eigenvalues, work, -1, info)
  lwork = int(work(1))
  deallocate(work)
  allocate(work(lwork))

  ! Solve eigenvalue problem
  call zheev('V', 'U', dimax, H_work, dimax, eigenvalues, work, lwork, info)
